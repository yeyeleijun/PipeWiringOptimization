...
class decomposition_heuristic(AStar):
    def __init__(self, maxit, space_coords, obstacle_coords, pipes, w_path, w_bend, w_energy, min_dis_bend):
        ...
        self.index_category = ['I_par']
        # self.index_category = ['I_par'] * int(0.1*maxit) + ['I_cluster'] * int(0.9*maxit)  # + ['I_seq'] * int(0.4*maxit)
    ...
    def main_run(self):
        ...
        while stop == 0 and it < self.maxit:
            ...
            for pipe_k in Kit:
                ...
                if self.index_category[it] == 'I_seq':
                    Kbar.remove(pipe_k)
                    self.update_cost(self.covering_list_n[k], change_cost=10)
            ...
            if not self.is_empty_list(self.cov_conflict):
                ...
                if self.index_category[it] == 'I_cluster':
                    ...
                    self.update_cost(list(conflict_edges), change_cost=50)
                    Kit = list(Kit_next)
                it = it + 1
            else:
                stop = 1
...
if __name__ == '__main__':
    ...
    pipes = []
    radia = [2, 2, 2, 1, 1]
    for m, coord in enumerate(np.arange(3, 20, 4)):
